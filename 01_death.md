...
It follows we therefore have no basis for saying: if Death is Nothing (<sup><sub>in the _phenomenal_ sense</sub></sup>), then Death precludes Being[^9], i.e. the conditional disjunction of `axiom of non-equivalence` and `axiom of precluded existence`. The conclusion we then draw is the mutually exclusion of Death and Being is not a relation of necessity, but incidence, and it could as well be the case the opposite is true. We cannot expunge this overlap from `solipsism`. What resides in this strange overlap of Being and Death, if not the _phenomenology of Death_? The problem `solipsism` wished to disperse emerges from the dust, like the eternal phoenix [^10]
...
[^8]: If we simultaneously _are_ while having _being Dead_, then it is indeed a strange type of Being we find ourselves inhabiting. For what else could follow, but to die again? 
[^9] The equivalence of "_not p implies q_" to "_p or q_" is called `conditional disjunction`; it can be seen clearly by constructing the truth tables for formulation. If we apply the algebra of logic here, we may rephrase as follows, "_If Death is not Being, then Death precludes Being_", and then shift forms into, "_Death is not Being or Death precludes Being_", where _or_ is used in the inclusive sense. Only one of these propositions need be true for the compound proposition to obtain. 
