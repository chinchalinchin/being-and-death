...
When philosophy attempts to hew to the scientific method, it necessarily becomes rationalism and reductionism. The project of philosophy in the modern world has become to explain away, not only consciousness, but Being itself, by reducing everything to representation and formulae, as if these representations and formulae did not exist as representation and formulae through our Being first, as if consciousness and Being were contained in the regression coefficient matrices underlying machine learning and not _their application over time and space_. 
...
For instance, we might model our brain's image recognition as a network that takes as its initial input the excitation levels of the red, blue and green cones in our eyes. The first level of inputs would take the form (R, G, B), and then the next layer of the network analyzes the cross-interactions of each input variable in the previous layer, and so on, until finally reaching a concept node that maps the initial input through the layers of the network to an concept such as _dog_, _cat_ or _boat_. 
