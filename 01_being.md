...
Where the symbol `#` represents the quantifier for existence, `@` represents the quantifier for universality, `!` represents negation, `|` represents denotion, `~` represents set membership, `<->` represents logical equivalence, uppercase letters A and B represents sets, lowercase letters represents unquantified set members, and n(S) represents the cardinality of a set S, i.e. its countability[^13]. With these definitions in hand, it then remains to show the cardinality of set _A_ is zero, and that should be self-evident from its definition, i.e. the set that asserts self-identity is impossible is empty, and thus we have the metaphysical basis of 0 and 1.
